...
## Демо-сценарии (Playwright)

Playwright используется для записи демо-сценариев — автоматизированных прогонов прототипа, которые показывают, как он работает.

- Демо-файлы лежат в `e2e/`. Каждый файл — один сценарий.
- Конфигурация: `playwright.config.ts` в корне. Браузер запускается видимым (headless: false), с замедлением (slowMo: 500ms) и записью видео.
- Прототип запускается автоматически при запуске демо (webServer в конфиге).

Правила создания демо:
1. Файл называй `e2e/<название>-demo.spec.ts`.
2. Сценарий должен быть наглядным: открыть страницу, выполнить действия, показать результат. Это не тесты — это демонстрация.
3. Используй `page.waitForTimeout(1000)` между шагами, чтобы зритель успевал видеть, что происходит.
4. Не используй assert/expect для проверки — это демо, а не тест.

## Команды

- `cd Demo_project && npm start` — запуск прототипа (порт 4200)
- `cd Demo_project && npm run build` — сборка
- `npm run demo` — запуск демо-сценариев (из корня проекта)
